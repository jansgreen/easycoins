...
import hmacSHA512 from '../cdnjs/crypto-js.js';
...
const Kucoin = {
    ...
    sign: function(endpoint, params, method) {
        ...
        let signatureResult = hmacSHA512(this.secretKey)
            .update(strForSign)
            .digest('base64')
        let passphraseResult = hmacSHA512(this.secretKey)
    ...
    formatQuery: function(queryObj) {
        if (JSON.stringify(queryObj).length !== 2) {
            return '?' + Market.stringify(queryObj)
        } else {
            return ''
        }
    }
